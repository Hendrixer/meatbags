...
- [x] 1.3 Write `src/db/schema.ts` defining `agents`, `tasks`, and `reviews` — `tasks.id` as a minted opaque id, `thread_id` unique and nullable, `args` as jsonb, `status` as a constrained text column, and `created_at`/`assigned_at` as separate columns; verify `tsc --noEmit` passes and the inferred row types expose every column.
...
- [ ] 2.2 Implement `createTask` (submit-time insert: id, tool name, args, description, status `pending`) and `getTask` (left-joined to `agents` so an unassigned task still returns); verify a script inserts a task and reads every field back unchanged with a null assignee.
- [ ] 2.3 Implement `dispatchTask(id, { agentId, threadId })` setting `assigned_at` and status `assigned`, and `findTaskByThread(threadId)`; verify dispatching a pending task then looking it up by thread id returns the same row, and that an unknown thread id returns nothing.
...
- [ ] 2.7 Implement the response-time recompute over completed tasks, measuring from `assigned_at`; verify a task dispatched and completed a known interval apart yields that interval, unaffected by how long it sat pending.
